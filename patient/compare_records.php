...
// Get patient_id from the logged-in user's session
$user_id = $_SESSION['user_id'];
$patientQuery = "SELECT patient_id, first_name, last_name, date_of_birth, gender, case_id FROM patients WHERE user_id = :user_id";
...
// Modify the recordsQuery to only fetch records that have corresponding data
$recordsQuery = "
    -- Appointment Records (only those with actual records)
    SELECT 
        ...
        NULL AS end_date,
        CASE 
            WHEN a.appointment_type = 'Regular Checkup' AND EXISTS (SELECT 1 FROM regular_checkup_records WHERE appointment_id = a.appointment_id) THEN 'regular_checkup'
            WHEN a.appointment_type = 'Follow Up' AND EXISTS (SELECT 1 FROM follow_up_records WHERE appointment_id = a.appointment_id) THEN 'follow_up'
            WHEN a.appointment_type = 'Under Observation' AND EXISTS (SELECT 1 FROM under_observation_records WHERE appointment_id = a.appointment_id) THEN 'under_observation'
            WHEN a.appointment_type = 'Pre Natal Checkup' AND EXISTS (SELECT 1 FROM prenatal_records WHERE appointment_id = a.appointment_id) THEN 'prenatal'
            WHEN a.appointment_type = 'Post Natal Checkup' AND EXISTS (SELECT 1 FROM postnatal_records WHERE appointment_id = a.appointment_id) THEN 'postnatal'
            WHEN a.appointment_type = 'Medical Consultation' AND EXISTS (SELECT 1 FROM medical_consultation_records WHERE appointment_id = a.appointment_id) THEN 'medical_consultation'
            WHEN a.appointment_type = 'Vaccination' AND EXISTS (SELECT 1 FROM vaccination_records WHERE appointment_id = a.appointment_id) THEN 'vaccination'
        END AS record_category
    FROM appointments a
    WHERE a.patient_id = :patient_id
    AND (
        (a.appointment_type = 'Regular Checkup' AND EXISTS (SELECT 1 FROM regular_checkup_records WHERE appointment_id = a.appointment_id))
        OR (a.appointment_type = 'Follow Up' AND EXISTS (SELECT 1 FROM follow_up_records WHERE appointment_id = a.appointment_id))
        OR (a.appointment_type = 'Under Observation' AND EXISTS (SELECT 1 FROM under_observation_records WHERE appointment_id = a.appointment_id))
        OR (a.appointment_type = 'Pre Natal Checkup' AND EXISTS (SELECT 1 FROM prenatal_records WHERE appointment_id = a.appointment_id))
        OR (a.appointment_type = 'Post Natal Checkup' AND EXISTS (SELECT 1 FROM postnatal_records WHERE appointment_id = a.appointment_id))
        OR (a.appointment_type = 'Medical Consultation' AND EXISTS (SELECT 1 FROM medical_consultation_records WHERE appointment_id = a.appointment_id))
        OR (a.appointment_type = 'Vaccination' AND EXISTS (SELECT 1 FROM vaccination_records WHERE appointment_id = a.appointment_id))
    )

    UNION ALL

    -- Transaction Records (only those with actual records)
    SELECT 
        ...
        NULL AS end_date,
        CASE 
            WHEN ms.service_name = 'Transvaginal Ultrasound' AND EXISTS (SELECT 1 FROM tv_ultrasound WHERE transaction_id = mt.transaction_id) THEN 'tv_ultrasound'
            WHEN ms.service_name = 'OB Ultrasound' AND EXISTS (SELECT 1 FROM ob_ultrasound WHERE transaction_id = mt.transaction_id) THEN 'ob_ultrasound'
            WHEN ms.service_name = 'Prenatal Checkup' AND EXISTS (SELECT 1 FROM prenatal_records WHERE transaction_id = mt.transaction_id) THEN 'prenatal'
            WHEN ms.service_name = 'Postnatal Checkup' AND EXISTS (SELECT 1 FROM postnatal_records WHERE transaction_id = mt.transaction_id) THEN 'postnatal'
            WHEN ms.service_name = 'Pap Smear' AND EXISTS (SELECT 1 FROM pap_smear WHERE transaction_id = mt.transaction_id) THEN 'pap_smear'
            WHEN ms.service_name = 'Urinalysis' AND EXISTS (SELECT 1 FROM urinalysis WHERE transaction_id = mt.transaction_id) THEN 'urinalysis'
            WHEN ms.service_name = 'Hemoglobin Test' AND EXISTS (SELECT 1 FROM hemoglobin WHERE transaction_id = mt.transaction_id) THEN 'hemoglobin'
            WHEN ms.service_name = 'Circumcision' AND EXISTS (SELECT 1 FROM circumcision_consent WHERE transaction_id = mt.transaction_id) THEN 'circumcision_consent'
            WHEN ms.service_name = 'Vaccination for Newborn' AND EXISTS (SELECT 1 FROM vaccination_records WHERE transaction_id = mt.transaction_id) THEN 'vaccination'
        END AS record_category
    ...
    AND mt.service_id != 10
    AND (
        (ms.service_name = 'Transvaginal Ultrasound' AND EXISTS (SELECT 1 FROM tv_ultrasound WHERE transaction_id = mt.transaction_id))
        OR (ms.service_name = 'OB Ultrasound' AND EXISTS (SELECT 1 FROM ob_ultrasound WHERE transaction_id = mt.transaction_id))
        OR (ms.service_name = 'Prenatal Checkup' AND EXISTS (SELECT 1 FROM prenatal_records WHERE transaction_id = mt.transaction_id))
        OR (ms.service_name = 'Postnatal Checkup' AND EXISTS (SELECT 1 FROM postnatal_records WHERE transaction_id = mt.transaction_id))
        OR (ms.service_name = 'Pap Smear' AND EXISTS (SELECT 1 FROM pap_smear WHERE transaction_id = mt.transaction_id))
        OR (ms.service_name = 'Urinalysis' AND EXISTS (SELECT 1 FROM urinalysis WHERE transaction_id = mt.transaction_id))
        OR (ms.service_name = 'Hemoglobin Test' AND EXISTS (SELECT 1 FROM hemoglobin WHERE transaction_id = mt.transaction_id))
        OR (ms.service_name = 'Circumcision' AND EXISTS (SELECT 1 FROM circumcision_consent WHERE transaction_id = mt.transaction_id))
        OR (ms.service_name = 'Vaccination for Newborn' AND EXISTS (SELECT 1 FROM vaccination_records WHERE transaction_id = mt.transaction_id))
    )

    UNION ALL

    -- Admission Records (only those that exist)
    SELECT 
    ...
    AND EXISTS (SELECT 1 FROM admissions WHERE admission_id = ad.admission_id)
...
<body>
    <div class="dashboard-container">
        ...
        <main class="dashboard-main-content">
            ...
            <div class="mt-4">
                <a href="manage_health_records.php" class="back-button">
