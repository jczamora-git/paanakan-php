...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$view_mode) {
    ...
    if (isset($_POST['record_id']) && $_POST['record_id']) {
        // Update
        $sql = "UPDATE medical_consultation_records SET chief_complaint=?, history_of_present_illness=?, physical_exam=?, diagnosis=?, treatment=?, prescription=? WHERE record_id=?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(array_merge(array_values($fields), [$_POST['record_id']]));
    } else {
        // Insert
        $sql = "INSERT INTO medical_consultation_records (appointment_id, chief_complaint, history_of_present_illness, physical_exam, diagnosis, treatment, prescription) VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(array_merge([$appointment_id], array_values($fields)));
    }
    // Handle complete appointment separately - check if this is a complete appointment request
    if (isset($_POST['complete_appointment']) && $_POST['complete_appointment'] === '1' && isset($_POST['confirm_complete']) && $_POST['confirm_complete'] === '1') {
        try {
        ...
        $stmt = $pdo->prepare("UPDATE appointments SET status = 'Done' WHERE appointment_id = ?");
            ...
            header("Location: medical_consultation.php?appointment_id=" . $_POST['appointment_id']);
            exit();
    ...
    header("Location: medical_consultation.php?appointment_id=$appointment_id");
    exit();
}
                ...
                <form method="post">
                    ...
                    <div class="form-grid">
                        <div class="mb-3">
                            ...
                            <textarea name="chief_complaint" class="form-control" rows="2" placeholder="Enter chief complaint..."><?= $existingRecord ? htmlspecialchars($existingRecord['chief_complaint']) : '' ?></textarea>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">History of Present Illness</label>
                            <textarea name="history_of_present_illness" class="form-control" rows="2" placeholder="Enter history of present illness..."><?= $existingRecord ? htmlspecialchars($existingRecord['history_of_present_illness']) : '' ?></textarea>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Physical Exam</label>
                            <textarea name="physical_exam" class="form-control" rows="2" placeholder="Enter physical exam..."><?= $existingRecord ? htmlspecialchars($existingRecord['physical_exam']) : '' ?></textarea>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Diagnosis</label>
                            <textarea name="diagnosis" class="form-control" rows="2" placeholder="Enter diagnosis..."><?= $existingRecord ? htmlspecialchars($existingRecord['diagnosis']) : '' ?></textarea>
                        </div>
                        <div class="mb-3 full-width">
                            <label class="form-label">Treatment</label>
                            <textarea name="treatment" class="form-control" rows="2" placeholder="Enter treatment..."><?= $existingRecord ? htmlspecialchars($existingRecord['treatment']) : '' ?></textarea>
                        </div>
                        <div class="mb-3 full-width">
                            <label class="form-label">Prescription</label>
                            <textarea name="prescription" class="form-control" rows="2" placeholder="Enter prescription..."><?= $existingRecord ? htmlspecialchars($existingRecord['prescription']) : '' ?></textarea>
                        </div>
                    </div>
